...
function dashboard(){let starts=0;const jobs=[];const tx={$queryRaw:async()=>[],checkout:{upsert:async()=>({id:'test-cart'})},callJob:{findFirst:async({where})=>jobs.find(j=>j.id===where.id),create:async({data})=>jobs.push(data),updateMany:async()=>({count:1})}};const db={...tx,$transaction:fn=>fn(tx)};const route=compile('app/routes/app.dashboard.tsx',{'react':{},'react/jsx-runtime':{},'react-router':{},'@shopify/shopify-app-react-router/server':{},'../shopify.server':{authenticate:{admin:async()=>({session:{shop:'shop'}})}},'../db.server':{default:db},'node:crypto':require('node:crypto'),'../callProvider.server':{startVapiCallForJob:async()=>{starts++}},'../lib/checkoutData.shared':{waitingReason:()=>null},'../callRecovery.server':{},'../components/dashboard/DashboardView':{}});return {route,jobs,starts:()=>starts};}
const request=(phone='+306900000000',intent='create_test_call')=>({formData:async()=>new Map(Object.entries({phone,intent,testCallId:'c8c7dce0-b2d4-4f4c-a464-26cc8f8fce75'}))});
