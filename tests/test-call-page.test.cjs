...
test('test page renders customer and cart fields immediately without opening a modal',()=>{
 ...
 const route=compile('app/routes/app.test-call.tsx',{'react':React,'react/jsx-runtime':require('react/jsx-runtime'),'node:crypto':require('node:crypto'),'react-router':{useLoaderData:()=>({testCallId:'test-id',storeName:'Afterwin',dashboardHref:'/app/dashboard'}),useActionData:()=>undefined,useNavigation:()=>({state:'idle'}),Form:props=>React.createElement('form',props)},'@shopify/polaris':Polaris,'../shopify.server':{},'../callProvider.server':{},'./app.dashboard':{}});
 ...
 for(const label of ['Afterwin','Product 1','Quantity','Unit price','Start test call'])assert.ok(html.includes(label));
