...
function fixture(env={}){
 let seq=0;const tables={privacyRequest:[],privacySuppression:[],supportInquiry:[],order:[],checkout:[],callJob:[],callCharge:[],smsDelivery:[],settings:[],session:[],shopBilling:[],billingCouponRedemption:[]};
 ...
 const seed=()=>{for(const [s,suffix] of [[shop,'a'],[other,'b']]){
  ...
  tables.smsDelivery.push({id:`sms-${suffix}`,shop:s,checkoutId:`100-${suffix}`});
 }};
...
test('customer erasure deletes related calls and charges, preserves other stores and invalidates old exports',async()=>{
 ...
 for(const t of ['order','checkout','callJob','callCharge','smsDelivery']){assert.equal(f.tables[t].length,1);assert.equal(f.tables[t][0].shop,other);}
...
test('shop redaction erases all local store data without touching another store',async()=>{
 ...
 for(const t of ['settings','shopBilling','billingCouponRedemption','supportInquiry','checkout','order','callJob','callCharge','smsDelivery'])assert.ok(f.tables[t].every(r=>r.shop!==shop));
});
