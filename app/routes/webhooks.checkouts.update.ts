...
export async function action({ request }: ActionFunctionArgs) {
  ...
  // IMPORTANT:
  // UPDATE event = customer activity => set OPEN and clear abandonedAt.
  // Then markAbandonedByDelay will set ABANDONED later if no further activity.
  ...
  try {
    ...
    try {
      ...
      console.warn("[CHECKOUTS_UPDATE] presentation enrichment deferred", { shop, checkoutId, error: String(error?.message ?? error) });
    }
