import { checkoutName, checkoutPhone, checkoutItems, shopifyCheckoutLabel, shopifyOrderLabel, unansweredCall, waitingReason } from "../lib/checkoutData.shared";
...
export const loader = async ({ request }: LoaderFunctionArgs) => {
  ...
  const rows: Row[] = checkouts.map((c) => {
    ...
    return {
      checkoutId,
      checkoutLabel: shopifyCheckoutLabel(c.raw, checkoutId),
...
export default function Checkouts() {
  ...
  const searchedRows = React.useMemo(() => filteredWorkRows.slice().sort((a, b) => Date.parse(b.createdAt) - Date.parse(a.createdAt) || b.checkoutId.localeCompare(a.checkoutId)).filter((r) =>
    [r.customerName, r.checkoutId, r.checkoutLabel, r.recoveredOrderNumber, r.cartPreview].some((v) => safeStr(v).toLowerCase().includes(query.trim().toLowerCase()))
                              ...
                              <s-table-cell style={compactCell}>
                                <s-stack gap="tight">
                                  <s-stack direction="inline" gap="tight" style={{ alignItems: "center", flexWrap: "wrap" }}>
                                    ...
                                    <s-text tone="subdued" variant="bodySm">Checkout {r.checkoutLabel}</s-text>
                  ...
                  <s-stack gap="base">
                    <s-stack direction="inline" align="space-between" gap="base" style={{ flexWrap: "wrap", alignItems: "center" }}>
                      <s-stack gap="tight">
                        ...
                        <s-text tone="subdued" variant="bodySm">
                          {selected ? selected.recoveredOrderNumber ? `Recovered order ${selected.recoveredOrderNumber} · Checkout ${selected.checkoutLabel}` : `Checkout ${selected.checkoutLabel}` : "Select a checkout"}
                        </s-text>
                      </s-stack>
