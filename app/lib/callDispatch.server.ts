...
async function claim(shop: string, id: string, manual: boolean): Promise<string | null> {
  return db.$transaction(async (tx) => {
    ...
    const job = await tx.callJob.findFirst({ where: { shop, id, ...(manual ? {} : { status: "QUEUED" as const }) } });
    if (!job || job.status === "CALLING") return null;
    ...
    else if (!manual && job.providerCallId) reason = "CALL_ALREADY_STARTED";
    else if (!manual && !settings?.enabled) {
    ...
    if (!manual && !reason && used >= attemptLimit(settings?.maxAttempts)) reason = "MAX_ATTEMPTS_REACHED";
    if (reason) {
      await tx.callJob.update({ where: { id }, data: { status: "CANCELED", outcome: reason } });
      return null;
    }
    if (history.some(row => row.id !== id && row.status === "CALLING")) return null;
    // A completed/failed call needs a new billing identity for an explicit retry.
    if (manual && (job.status !== "QUEUED" || job.providerCallId)) {
      const created = await tx.callJob.create({ data: { shop, checkoutId: job.checkoutId, phone: checkout!.phone || job.phone, scheduledFor: new Date(), status: "CALLING", attempts: 1 } });
      return created.id;
    }
...
export async function claimCallJob(shop: string, id: string) {
  return Boolean(await claim(shop, id, false));
}

export async function claimManualCallJob(shop: string, id: string) {
  return claim(shop, id, true);
}
