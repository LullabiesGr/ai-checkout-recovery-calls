export function parseTestCallInput(fd: { get(name: string): unknown }) {
  ...
  if (!["EUR", "USD", "GBP", "CAD", "AUD"].includes(currency)) throw new Error("Select a supported currency.");
  let items: unknown;
  ...
  const normalized = items.map((item: any) => {
    ...
    if (!title || title.length > 200 || !Number.isInteger(quantity) || quantity < 1 || quantity > 1000 || !Number.isFinite(price) || price <= 0 || price > 100000) throw new Error("Each product needs a name, a quantity from 1 to 1,000, and a positive unit price.");
